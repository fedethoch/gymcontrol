...
export function MobileTabBar({ isAuthenticated, role }: MobileTabBarProps) {
  ...
  return (
    ...
    >
      <div className="grid min-h-[4.75rem] grid-cols-5">
        {tabs.map((tab) => {
          ...
          return (
            <Link
              ...
              href={tab.href}
              className={cn(
                "flex flex-col items-center justify-center gap-1.5 px-1 py-3 text-xs font-semibold transition-colors",
        ...
        <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
          <SheetTrigger asChild>
            <button
              type="button"
              className="flex flex-col items-center justify-center gap-1.5 px-1 py-3 text-xs font-semibold text-[#8a93ad] transition-colors hover:text-white"
            >
