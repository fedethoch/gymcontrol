...
type RecipeFormProps = {
  ...
  initialName?: string;
  /** Alimentos propios y del catálogo; la receta guarda nombre y macros de cada ingrediente. */
  foods: Food[];
  onSaved: (recipe: Recipe) => void;
  onCancel: () => void;
};

type DraftIngredient = { foodId: string; foodName: string; grams: string; food: RecipeFoodNutrition };
...
export function RecipeForm({ recipe, initialName = "", foods, onSaved, onCancel }: RecipeFormProps) {
  ...
  const [createFoodRequest, setCreateFoodRequest] = useState<FoodCreateRequest | null>(null);
  // Alimentos creados desde este formulario: todavía no están en `foods` (viene del server).
  ...
  const preview = useMemo(() => {
    ...
    const input = {
      ...
      ingredients: ingredients.map((ingredient) => ({ grams: toNumber(ingredient.grams) ?? 0, food: ingredient.food })),
    };
  ...
  }, [ingredients, servingG, totalWeightG]);
  ...
  function addIngredient(food: Food) {
    setIngredients((current) => [
      ...current,
      {
        foodId: food.id,
        foodName: food.name,
        grams: String(food.servingG),
        food: { servingG: food.servingG, calories: food.calories, proteinG: food.proteinG, carbsG: food.carbsG, fatG: food.fatG },
      },
    ]);
    setQuery("");
  }
  ...
  function handleFoodCreated(food: Food) {
    setCreatedFoods((current) => [food, ...current]);
    addIngredient(food);
    setCreateFoodRequest(null);
  }
  ...
  return (
    <>
        ...
        <div className="grid gap-2">
            ...
            {query.trim() ? (
              <div className="absolute inset-x-0 top-full z-10 mt-1 overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-lg">
                ...
                <button
                  type="button"
                  className={cn(
                    "flex min-h-11 w-full items-center gap-2 px-3 py-2 text-left text-sm font-medium text-[var(--accent)] hover:bg-[var(--card-alt)]",
                    matches.length > 0 && "border-t border-[var(--border)]",
                  )}
                  onClick={() => setCreateFoodRequest({ key: Date.now(), name: query.trim() })}
                >
                  <Plus aria-hidden="true" className="size-4 shrink-0" />
                  <span className="truncate">Crear alimento «{query.trim()}»</span>
                </button>
              </div>
            ) : null}
          </div>
          ...
          {ingredients.length === 0 ? (
            <p className="rounded-xl border border-dashed border-[var(--border)] px-3 py-3 text-center text-xs text-[var(--foreground-muted)]">
              Buscá y agregá los alimentos que lleva la receta, en crudo. Si no está, crealo desde el buscador.
            </p>
          ) : (
      ...
      {/* Fuera del <form>: los eventos de React cruzan el portal y el submit del alimento guardaría la receta. */}
      <FoodCreateDrawer
        request={createFoodRequest}
        description="Queda en tus alimentos. En la receta se ven su nombre y sus valores."
        onClose={() => setCreateFoodRequest(null)}
