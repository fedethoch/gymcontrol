...
// ─────────────────────────────────────────────────────────────────────────────
// Vendored from react-body-highlighter@2.0.5 — npm install fails on React 19.
// SVG polygon paths, utils, and types are inlined here. Data is unmodified.
// ─────────────────────────────────────────────────────────────────────────────
...
const DEFAULT_MUSCLE_DATA: Record<Muscle, IMuscleData> = {
...
function fillMuscleData(data: IExerciseData[]): Record<Muscle, IMuscleData> {
  return data.reduce(
    (acc, exercise) => {
    ...
    JSON.parse(JSON.stringify(DEFAULT_MUSCLE_DATA)) as Record<Muscle, IMuscleData>,
  );
}

function fillIntensityColor(
  activityMap: Record<Muscle, IMuscleData>,
  highlightedColors: string[],
  muscle: Muscle,
): string | undefined {
  const frequency = activityMap[muscle]?.frequency;
  if (frequency == null || frequency === 0) return undefined;
...
// intensity ratio → frequency bucket (1-4, maps to highlightedColors index)
function intensityToFreq(count: number, max: number): number {
...
// ── Highlighted colors: low → high ───────────────────────────────────────────
const HIGHLIGHTED_COLORS = ["#16a34a", "#ca8a04", "#ea580c", "#dc2626"];
const BODY_COLOR = "#263347";

// ── Inline SVG body renderer ──────────────────────────────────────────────────
function BodySVG({
  modelData,
  muscleMap,
  muscleColors,
  ...
  return (
    ...
    <svg viewBox="0 0 100 200" width="90" height="180" xmlns="http://www.w3.org/2000/svg">
      {modelData.map(({ muscle, svgPoints }) =>
        svgPoints.map((points, i) => (
          <polygon
            key={`${muscle}-${i}`}
            points={points}
            fill={
...
export function BodyMuscleFigure({
  ...
  const exerciseData: IExerciseData[] = Object.entries(muscleLoad).flatMap(([key, count]) => {
    const muscles = toLibraryMuscles(key);
    if (muscles.length === 0) return [];
    return [{ name: key, muscles, frequency: intensityToFreq(count, maxCount) }];
  });
