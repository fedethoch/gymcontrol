...
| Avisos push (qué, cuándo, envío, cron) | `app/lib/notifications.ts`, `app/lib/rest-push.ts`, `app/lib/weekly-summary.ts` (lógica pura + `tests/unit/`), `app/lib/push/` (server-only), `app/api/push/`, `public/sw.js`, `app/lib/push-client.ts`; UI S7 en `app/components/configuracion/`; migraciones `20260921_push_*`; reglas en `DESIGN.md` §6.4, datos en `docs/DATABASE.md` |
