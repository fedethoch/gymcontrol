...
| Avisos push (`app/lib/notifications.ts`, `rest-push.ts`, `app/lib/push/`, `public/sw.js`, S7) | `pnpm test:unit` + `pnpm build && pnpm start` (el SW solo existe en producción): Playwright con CDP `ServiceWorker.deliverPushMessage`, S7 con `PushManager` falso; push real con Chrome en perfil persistente (`launchPersistentContext`, sin `--disable-background-networking`) + `curl` a `/api/push/cron` con `CRON_SECRET`; iPhone real para iOS. Restaurar preferencias y borrar filas de prueba de la cuenta admin |
