...
| Comida (una por comida) | a la hora de cada comida (pedido del usuario, 2026-09-22): desayuno 08:00 · almuerzo 12:30 · merienda 17:30 · cena 21:00 | esa comida sigue sin ítems hoy y hay perfil de nutrición | el "+" de esa comida en el registro |
...
- Las horas de la tabla son las de por defecto: cada una se cambia en S7.
- Sin aviso de prueba: "Probar notificación" se sacó a pedido del usuario (2026-09-22).
...
| Pieza | Regla |
|---|---|
| Sugerencia | Doble progresión **por serie** contra el objetivo del plan: cada serie compara con la misma serie de antes (si llegó al tope sube la carga solo en esa serie, si no suma una rep), así que cada serie tiene su propio objetivo, su texto ("Hoy: buscá 40 kg × 10.") y su precarga. En desktop el texto nombra la serie ("Serie 2 · …"). Primera vez en el día, "Primera vez en este día: apuntá a {objetivo}" |
| Progresión semanal | Se progresa una vez por semana (lunes a domingo) por entreno. Serie por serie: la primera sesión de la semana apunta a superar la mejor marca de esa serie en la última semana entrenada; si el entreno se repite en la semana, cada serie reintenta su objetivo ("Otra chance: …") o, si ya lo logró, reafirma su mejor marca de la semana ("Reafirmá tu marca: …"); textos de una línea para no sumar alto a la pantalla sin scroll. `planWeeklyProgression` en `app/lib/workout-progression.ts` (2026-09-21) |
...
| Zona | Contenido |
|---|---|
| Cabecera | Nombre en H1 y `Grupo · Equipamiento` en muted. Debajo, segmentado de 48px `Técnica · Historial` (activo en `--card-alt` con borde, nunca emerald). Solo aparece si el ejercicio tiene historial en ese día (§11.3 Marcas por día; hoy, solo en `/rutinas/dia`); los chips "Técnica" e "Historial" del pager abren el sheet en su pestaña |
...
| S4 Objetivo | 3 capas para no marear (2026-09-17, `app/lib/nutrition-plan-options.ts`). Vista principal: 3 grupos (Déficit · Mantenimiento · Ganancia) con las kcal que daría cada uno con su variante; debajo `SegmentedControl` de variantes con "★" en la recomendada (Suave/Moderado★/Agresivo · Recomposición★/Mantener · Limpia★/Estándar/Agresiva), el hint de la elegida y la consecuencia en mono (`−20% · −522 kcal`). "Ajustar intensidad" (texto) despliega un `Slider` de 1% con la franja de las variantes y "Volver a lo recomendado". Después la vista previa del peso a 12 semanas (número + forma de la curva, alto fijo; también en el paso 6 del alta, que solo muestra los 3 grupos) y las filas de 72px "Tipo de dieta" y "Avanzado", que abren vistas internas con flecha atrás. En manual: aviso "Fijado a mano" arriba que lleva a Avanzado |
| S4a Tipo de dieta | 6 filas: Equilibrada★ · Alta en proteína · Alta en carbos · Alta en grasas · Keto · Personalizada, con hint de una frase, reparto `P·C·G %` en mono y la etiqueta "Sugerida" (neutra) en la que corresponde al objetivo si no es la ★. Con Personalizada: 3 sliders enlazados (suma fija = kcal objetivo; mover P o G lo absorben los carbos; mover C lo absorbe la grasa; en el límite el thumb se frena). Cada uno: gramos en Metric M, % y g/kg en mono, franja recomendada en el track, relleno con su `MACRO_COLORS`. "Volver a Equilibrada" y, con carbos en el mínimo, "¿Querés Keto?" |
...
| S7 Notificaciones | Arriba, el estado del dispositivo: "Instalá la app para recibir avisos" (iPhone en Safari: 3 pasos, y que adentro hay que iniciar sesión otra vez), "Enterate sin abrir la app" + lista de 4 avisos + "Activar notificaciones" (único CTA emerald; pide el permiso), "Los avisos están bloqueados" (dónde activarlos en iPhone y Android) o, activadas, tarjeta "Activadas en este iPhone/celu" con la fila "Desactivar" de 52px abajo, separada por una línea (campana tachada centrada bajo el ícono de arriba, texto alineado con el título; como "Cerrar sesión" de Z4). Activadas, debajo: Entrenamiento (Hoy toca entrenar + hora, Fin del descanso), Comidas (4 filas con hora y switch; "Solo te avisamos si esa comida sigue sin registrar.") y Resumen semanal (switch, chips Lu–Do como §12.3 y fila Hora). La hora se muestra en HH:MM en una pastilla mono de 36px que se ajusta al texto (apagado: atenuada). No se deja dibujar al `<input type="time">`: con reloj de 12 h iOS escribe "10:00 AM" a 16px y no entra. El input queda invisible encima, con área táctil de 44px, y abre la rueda del sistema (con mouse, `showPicker()`). Switch de 44×26 (`--accent` prendido, `--border-strong` apagado) con área de 48×44. Guarda solo (800 ms y al cerrar). Pie: versión de avisos (service worker). `/configuracion?panel=notificaciones` la abre (campanas de Inicio y del header) y al cerrar se limpia la URL |
